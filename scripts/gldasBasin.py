...
class GLDASData():
    ...
    def loadStudyData(self, reloadData=False, masking=True):
        ...
        if reloadData:  
            ...
            np.save('{0}_twsdata'.format(self.watershed), [self.nldastwsB, self.precipMat, self.gldasmask, self.sm200])            
        else:
            self.nldastwsB, self.precipMat, self.gldasmask, self.sm200 = np.load('{0}_twsdata.npy'.format(self.watershed))
