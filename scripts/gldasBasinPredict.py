...
class NDVIData():
    ...
    def __init__(self, watershed, reLoad=False):
        if reLoad:
            ...
            np.save('{0}NDVI_{1}.npy'.format(PREDICT_DIR, watershed), [self.ndviData])
        else:
            self.ndviData = np.load('{0}NDVI_{1}.npy'.format(PREDICT_DIR, watershed))
    ...
    def loadNDVI(self, extents, startYear=2002, endYear=2017):
...
class GLDASData():
    ...
    '''
    def __init__(self, watershed, watershedInner, startYear=2000, endYear=2017):
        '''
    ...
    def loadStudyData(self, reloadData=False, masking=True):
        ...
        if reloadData:  
            ...
            self.nldastwsB = tws
            print ('nldastws shape', self.nldastwsB.shape)
            ...
            np.save('{0}/{1}_twsdata.npy'.format(PREDICT_DIR, self.watershed), [self.nldastwsB, self.precipMat, self.gldasmask, self.sm200])            
        else:
            self.nldastwsB, self.precipMat, self.gldasmask, self.sm200 = np.load('{0}/{1}_twsdata.npy'.format(PREDICT_DIR,self.watershed))
        ...
        #as 05022018, define training validation range
        self.tvRange= np.arange((2002-self.startYear)*12+3, (2016-self.startYear+1)*12)-((2002-self.startYear)*12+3)
        print (self.sm200.shape)
    ...
    def extractVar(self, varname, extents, startYear=2000, endYear=2017):
        '''
    ...
    def formMatrix2D(self, n_p=3, masking=False, nTrain=1000):
        '''
    ...
    def getTWSAvg(self, mat, extents, subbasin=None):
        ...
        for i in range(mat.shape[0]):
            ...
            if subbasin is None:
                res = np.multiply(res,self.innermask)
                twsavg[i] = np.nansum(res)/self.nActualCells
            else:
                if subbasin=='indus':                    
...
def main():
    ...
    Xtrain,Xtest = gldas.formMatrix2D(n_p=3, masking=isMasking, nTrain=106)
