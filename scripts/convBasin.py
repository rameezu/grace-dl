...
def modelTrain(model, retrain, watershedName, label, Xin, Yin, weightfile):
    ...
    if retrain:     
        ...
        history = model.fit(Xin, Yin,
                            ...
                            shuffle=True,
                            verbose=1, callbacks=[reduce_lr] ) #callbacks=[reduce_lr, early_stop]
...
def CNNCompositeDriver(watershedName, watershedInner, n_p=3, nTrain=125, modelOption=1, retrain=False):
    ...
    doTesting(model, label, nldas, X_train, X_test, Y_test, Xp_train, Xp_test, n_p=n_p, nTrain=nTrain)
...
def CNNDriver(watershedName, watershedInner, n_p=3, nTrain=125, retrain=False, modelOption=1):
    ...
    doTesting(model, label, nldas, X_train, X_test, Y_test, n_p=n_p, nTrain=nTrain, pixel=False) 
...
def UnetDriver(watershedName, watershedInner, retrain=False, n_p=3, nTrain=125, modeloption=1):
    ...
    if modeloption==1:
        ...
        doTesting(model, label, nldas, X_train, X_test, Y_test,  Xp_train, Xp_test, n_p=n_p, nTrain=nTrain, pixel=False)
    elif modeloption==3:
        doTesting(model, label, nldas, X_train, X_test, Y_test, Xp_train, Xp_test,
...
if __name__ == "__main__":
    ...
    if option==1:
        ...
        main(retrain=retrain, modelnum=5, modeloption=2)#vgg16-3, composite, P, NDVI, vgg16, trained
    elif option==4:
        ...
        main(retrain=retrain, modelnum=4, modeloption=3)#unet-3, tws, P, NDVI trained
